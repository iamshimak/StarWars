...
internal final class BaseAPIClient: APIClient {
    ...
    func download<Data>(
        url: String,
        completion: @escaping DataCompletionCallback<Data>
    ) {
        networkProvider.download(url: url) { [weak self] result in
            guard let self = self else { return }
            
            switch result {
            case .success(let response):
                self.handleData(response, completion: completion)
    ...
    private func handleData<Data>(
        _ response: Network.Response,
        completion: DataCompletionCallback<Data>
    ) {
        do {
            guard let data = response.data, !data.isEmpty else {
                guard emptyDataStatusCodes.contains(response.statusCode) else {
            ...
            completion(.success(data as? Data), response.headers)
        } catch let error {
            completion(
                .failure(handleCustomAPIError(from: response) ?? error),
                response.headers
